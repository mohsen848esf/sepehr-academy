...
import { getUserID ,getCurrentUser , postCurrentUser } from '../../../services/storage/storage';
...
class CoursesList extends Component {
    ...
    getPagedData = () => {
        ...
        if (searchQuery){
            filtered = allCourses.filter(c =>
                c.teacher.fullName.startsWith(searchQuery) || c.course.courseName.startsWith(searchQuery) || c.course.topics.find(t => t.startsWith(searchQuery))
            );
        }
    ...
    render() {
        ...
        return (
            <Fragment>
                {selectedTab["_id"] === "courses-list" && count === 0 ? <p>هیچ دوره ای ثبت نشده است.</p> :
            <div className={selectedTab["_id"] === "courses-list" ? "tab-pane fade allCourses-panel show active" : "tab-pane fade allCourses-panel "} id="v-pills-allCourses" role="tabpanel" aria-labelledby="v-pills-allCourses-tab">
